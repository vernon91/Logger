...
namespace Logger
{
	public abstract class LoggerBase : NotificationObject
	{
		...
		public void Log(string format, params object[] args)
		{
			...
			try
			{
                ...
                string subFolder = "Logging";

                string fullPath = Path.Combine(mainFolder, subFolder);

                using (var w = File.AppendText(fullPath + @"\" + this.Filename))
				{
	...
	public class BattleLog : LoggerBase
	{
		public BattleLog(KanColleProxy proxy)
		{
            ...
            proxy.api_req_combined_battle_battleresult.TryParse<kcsapi_combined_battle_battleresult>().Subscribe(x => this.BattleResultCombined(x.Data));
		...
		private void BattleResult(kcsapi_battleresult br)
		{
            ...
            Log("{0},{1},{2},{3},{4}", this.TimestampISO(), 
                br.api_get_ship.api_ship_name, br.api_quest_name, br.api_enemy_info.api_deck_name, br.api_win_rank);
		}

        private void BattleResultCombined(kcsapi_combined_battle_battleresult br)
        {
            if (br.api_get_ship == null)
                return;
    ...
    public class ExpedLog : LoggerBase
    {
        public ExpedLog(KanColleProxy proxy)
        {
            proxy.api_req_mission_result.TryParse<kcsapi_mission_result>().Subscribe(x => this.ExpedResult(x.Data));
            this.Text = "Expedition";
            this.Filename = "ExpedLog.csv";
            this.LoggerName = "ExpedResults";
            this.LoadSettings();
        }
